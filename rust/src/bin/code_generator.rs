...
fn generate_tests(functions: &[FunctionComplexity]) -> Result<TokenStream> {
    ...
    for func in functions {
        ...
        let complexity_comment = format!("Test for {} (complexity: {})", 
...
fn generate_reports(functions: &[FunctionComplexity]) -> Result<TokenStream> {
    ...
    let high_complexity_count = functions.iter()
        .filter(|f| matches!(f.return_complexity, 
    ...
    let avg_cyclomatic: f64 = if total_functions > 0 {
        functions.iter().map(|f| f.cyclomatic_complexity).sum::<usize>() as f64 / total_functions as f64
    } else {
        0.0
    };
